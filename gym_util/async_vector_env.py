...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset_async(self, seed=None):
        ...
        for pipe in self.parent_pipes:
            ...
            # pipe.send(("reset"))  # send empty dict to match the signature of reset in gym envs
        self._state = AsyncState.WAITING_RESET

    def reset_wait(self, seed=None, timeout=None):
        """
        Parameters
        ----------
        ...
        if not self.shared_memory:
            ...
            # self.observations = concatenate(
            #     list(results), self.observations, self.single_observation_space
            # )
